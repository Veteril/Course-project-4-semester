...
wsp::MFTParser::MFTParser(const MFTInfo& mftInfo)
...
void wsp::MFTParser::readFirstRecord()
{
...
inline BOOLEAN wsp::MFTParser::seek(UINT64 uDistance, DWORD dwMoveMethod) const
{
...
const std::shared_ptr<wsp::VolumeInfo> wsp::MFTParser::getVolumeInfo() const
{
	return m_pVolumeInfo;
}

// checks if record corresponds to $Volume file, if so, saves volume attributes
BOOLEAN wsp::MFTParser::findVolumeAttributes(MFTEntryHeader* pHeader, UINT64 uAddress)
{
...
void wsp::MFTParser::findDeletedFiles()
{
 ...
 */
BOOLEAN wsp::MFTParser::checkForDeleted(wsp::MFTEntryHeader* pHeader, UINT64 uRecordAddress)
{
...
const std::shared_ptr<std::list<wsp::DeletedFile>> wsp::MFTParser::getDeletedFiles() const
{
	return m_pDeletedFiles;
}

inline WCHAR* wsp::MFTParser::readUtf16String(CHAR* pSource, WORD wLength) const
{
...
void wsp::MFTParser::iterate(PointerToMemberFunction jobFunc)
{
...
BOOLEAN wsp::MFTParser::undelete(const wsp::DeletedFile* const pDeletedFile, LPCWSTR lpszDirectoryName) const
{
...
void wsp::MFTParser::undeleteFile(HANDLE hFile, const AttributeHeader* pAttrHeader) const
{
	DWORD dwNumberOfBytes;
...
BOOLEAN wsp::MFTParser::findBitmap(MFTEntryHeader* pHeader, UINT64 uAddress)
{
	const AttributeHeader* pAttrHeader = (AttributeHeader*)((CHAR*)pHeader + pHeader->m_wAttributeOffset);
...
inline BOOLEAN wsp::MFTParser::intersect(const Runlist* const pLhs, const Runlist* const pRhs, WORD wLhsRunIndx, WORD wwRhsRunIndx) const
{
...
BOOLEAN wsp::MFTParser::isDataCleared(const AttributeHeader* pAttrHeader) const
{
...
BOOLEAN wsp::MFTParser::checkIfAllocated(UINT64 uNumberOfClusters, UINT64 uOffsetInBytes, UINT64 uOffsetInBits) const
{
	static const DWORD dwClusterSize = m_pMft->m_cSectorsPerCluster * SECTOR_SIZE;
...
*/
inline const wsp::AttributeHeader* wsp::MFTParser::findAttr(const AttributeHeader* pAttrHeader, UINT32 uAttrType) const
{
...
void wsp::MFTParser::undeleteDirectory(MFTEntryHeader* pHeader) const
{
	const AttributeHeader* pAttrHeader = (AttributeHeader*)((CHAR*)pHeader + pHeader->m_wAttributeOffset);
...
wsp::MFTParser::MFTParser(MFTParser&& rhs) noexcept
