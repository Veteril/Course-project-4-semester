...
wsp::Interface::Interface(Controller& controller)
	: m_Controller(controller)
{
	CoInitializeEx(NULL, COINIT_APARTMENTTHREADED | COINIT_DISABLE_OLE1DDE);
}

void wsp::Interface::start()
{
	while (1)
	{
...
void wsp::Interface::outputVolumesInformation() const
{
...
std::ostream& wsp::Interface::printVolumeInfo(std::ostream& os, const MFTInfo& mftInfo, const VolumeInfo& volumeInfo) const
{
...
std::string wsp::Interface::getInput(std::ostream& os, std::istream& is, const char* pszMessage) const
{
	os << pszMessage;
	std::string input;
	getline(is, input);
...
int wsp::Interface::findDeletedFiles()
{
...
void wsp::Interface::printDeletdFilesInfo(const std::shared_ptr<std::list<DeletedFile>> pDeletedFiles)
{
...
int wsp::Interface::undelete(int nVolumeNumber)
{
...
bool wsp::Interface::isNumber(const std::string& text) const
{
	return std::find_if(text.cbegin(), text.cend(), [](char c) { return !isdigit(c); }) == text.cend();
}

int wsp::Interface::getVolumeNumber() const
{
	int volumeNumber = -1;
	std::string input;
...
long long int wsp::Interface::getNumberOfFile(int nVolumeNumber) const
{
	std::string input;
...
const std::string wsp::Interface::getDirectory() const
{
